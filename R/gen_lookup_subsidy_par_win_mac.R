                                           ...
                                           num_clusters = detectCores()-2)
{
  ...
  FN_optim2 = function(jj){
    foo_dt1 = foo_irr[Well_ID_grp == jj & quarter == 1,
                      .(group_1, quarter, group_2, irrigation, profit)]
    ...
    foo_dt4 = foo_irr[Well_ID_grp == jj & quarter == 4,
                      .(group_1, quarter, group_2, irrigation, profit)]
    foo_dt3 = merge(foo_dt3, foo_dt4, by = c("group_1"), allow.cartesian = T)
    foo_dt2 = merge(foo_dt2, foo_dt3, by = c("group_1"), allow.cartesian = T)
    setnames(foo_dt2, old = c("quarter.x",  "group_2.x",  "irrigation.x",  "profit.x"),
             new = c("quarter.xx", "group_2.xx", "irrigation.xx", "profit.xx"))
    setnames(foo_dt2, old = c("quarter.y",  "group_2.y",  "irrigation.y",  "profit.y"),
             new = c("quarter.yy", "group_2.yy", "irrigation.yy", "profit.yy"))
    foo_dt1 = merge(foo_dt1, foo_dt2, by = c("group_1"), allow.cartesian = T)
    foo_dt1[, group_3 := .GRP, by= c("group_2.x", "group_2.y", "group_2.xx", "group_2.yy")]
    setkey(foo_dt1, group_3)
    rm(foo_dt2, foo_dt3, foo_dt4)
    foo_dt1[, irrigation_sum := irrigation.x + irrigation.y + irrigation.xx + irrigation.yy]
    foo_dt1[, `:=`(irrigation_below, ifelse(irrigation_sum <
                                              subsidy_threshold, subsidy_threshold - irrigation_sum,0))]
    foo_dt1[, profit_sum := profit.x + profit.y + profit.xx + profit.yy]
    foo_dt1[, `:=`(profit_sum_sub, profit_sum + irrigation_below * subsidy_amount)]

    foo_dt1[, mean_profit_combination     := mean(profit_sum),     by=c("group_3")]
    foo_dt1[, mean_profit_combination_sub := mean(profit_sum_sub), by=c("group_3")]
    foo_dt1[, mean_irrigation_combination := mean(irrigation_sum), by=c("group_3")]
    ...
    foo_dt4 = foo_dt1[,.(group_1, quarter = quarter.yy, group_2 = group_2.yy, mean_irrigation_combination, mean_profit_combination, mean_profit_combination_sub)]
    foo_dt3 = foo_dt1[,.(group_1, quarter = quarter.xx, group_2 = group_2.xx, mean_irrigation_combination, mean_profit_combination, mean_profit_combination_sub)]
    foo_dt2 = foo_dt1[,.(group_1, quarter = quarter.y,  group_2 = group_2.y,  mean_irrigation_combination, mean_profit_combination, mean_profit_combination_sub)]
    foo_dt1 = foo_dt1[,.(group_1, quarter = quarter.x,  group_2 = group_2.x,  mean_irrigation_combination, mean_profit_combination, mean_profit_combination_sub)]
  ...
  for (i in 1:length(unique_soil)) {
    ...
    if (Sys.info()[1] == "Windows"){
      ...
      foo_dt_all <- parLapply(cl, 1:aa, FN_optim2)
      stopCluster(cl)
    } else {# mclapply() for everybody else
      print(Sys.info()[1])
      foo_dt_all <- mclapply(X=1:aa, FUN=FN_optim2, mc.cores=num_clusters)
    }
