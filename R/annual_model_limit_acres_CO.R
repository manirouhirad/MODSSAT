...
#' @param irrigated_policy_limit         is the the maximum number of quarter circles that each well can irrigate. 
...
#' }
#' @export
annual_model_limit_acres_CO = function (soil_weather_file   = "./input_files/Well_SoilType_WeatherStation_CO.csv",
                                  ...
                                  well_capacity_intervals = 20)
{
  ...
  irrigated_policy_limit_dt = fread(irrigated_policy_limit)
  setkey(lookup_table_all_years_2,  Well_ID)
  setkey(irrigated_policy_limit_dt, Well_ID)
  lookup_table_all_years_2 = irrigated_policy_limit_dt[lookup_table_all_years_2]
  lookup_table_all_years_2 = lookup_table_all_years_2[tot_quarters <= QCircle_limit_policy]
  ...
  econ_output = lookup_table_all_years_2[, .(Well_ID, well_capacity = well_capacity_org,
                                             tot_acres, quarter, CR, PAW, irrigation_ac_in = irrigation, irrigation_depth_mm = round(irrigation/32.5 * 25.4), profit, yield_bu_ac)]
  econ_output[, `:=`(row, 1:.N)]
  write.csv(econ_output, well_capacity_file_year, row.names = FALSE)
  
  econ_output = lookup_table_all_years_2[, .(Well_ID, well_capacity = well_capacity_org,
                                             tot_acres, IFREQ, quarter, CR, PAW, irrigation_ac_in = irrigation, irrigation_depth_mm = round(irrigation/32.5 * 25.4), profit, yield_bu_ac)]
  econ_output[, `:=`(row, 1:.N)]
  econ_output = econ_output[,.(Well_ID, year = year_dt, well_capacity, tot_acres, IFREQ, quarter, CR, PAW, irrigation = irrigation_ac_in, profit, yield_bu_ac, row)]
