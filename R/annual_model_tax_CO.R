...
annual_model_tax_CO = function(tax_amount = 1,
                            # soil_weather_file = "./input_files/Well_Soil Type.csv",
                            ...
                            irrigation_season_days = 70)
{
  ...
  well_capacity_data[Well_capacity > `Well_Capacity(gpm)`, Well_capacity := `Well_Capacity(gpm)`]
  ...
  well_capacity_data[Well_capacity >= maximum_well_capacity, `:=`(Well_capacity, maximum_well_capacity)]
  ...
  # lookup_table_all_years_2[, `:=`(irr_tot_acres, sum(irrigation)), by = "Well_ID"]
  # lookup_table_all_years_2[, `:=`(profit_Well_ID, sum(profit)),    by = "Well_ID"]
  # lookup_table_all_years_2 = unique(lookup_table_all_years_2,      by = "Well_ID")
  
  # lookup_table_all_years_2[, `:=`(output_rate_acin_day, irr_tot_acres/irrigation_season_days)]
  econ_output = lookup_table_all_years_2[, .(Well_ID, well_capacity = well_capacity_org, tot_acres, quarter, CR, PAW, irrigation, profit)]
  econ_output[, `:=`(row, 1:.N)]
  econ_output[, `:=`(tax, tax_amount)]
  
  write.csv(econ_output, well_capacity_file_year, row.names = FALSE)
  
  # CO_DSSAT_output = econ_output[1]
  # CO_DSSAT_output[, Well_ID := 0]
  # write.csv(CO_DSSAT_output, "./input_files/CO_DSSAT_output.csv", row.names = F)
