...
)
{
  ...
  FN_optim2 = function(jj) {
    ...
    foo_dt3 = foo_irr[Well_ID_grp == jj & quarter == 3, .(Well_ID, SDAT, tot_acres, CR_3 = CR, PAW_3 = PAW, irrigation_3 = irrigation, profit_3 = profit)]
    foo_dt4 = foo_irr[Well_ID_grp == jj & quarter == 4, .(Well_ID, SDAT, tot_acres, CR_4 = CR, PAW_4 = PAW, irrigation_4 = irrigation, profit_4 = profit)]
    foo_dt3 = merge(foo_dt3, foo_dt4, by = c("Well_ID", "SDAT", "tot_acres"),
                    allow.cartesian = T)
    foo_dt2 = merge(foo_dt2, foo_dt3, by = c("Well_ID", "SDAT", "tot_acres"),
                    allow.cartesian = T)
    foo_dt1 = merge(foo_dt1, foo_dt2, by = c("Well_ID", "SDAT", "tot_acres"),
                    allow.cartesian = T)
    # foo_dt1[, `:=`(group_3, .GRP), by = c("Well_ID", "tot_acres", "SDAT", "quarter_1", "quarter_2", "quarter_3", "quarter_4")]
    # setkey(foo_dt1, group_3)
    rm(foo_dt2, foo_dt3, foo_dt4)
    foo_dt1[, `:=`(irrigation_sum, irrigation_1 + irrigation_2 + irrigation_3 + irrigation_4)]
    foo_dt1[, `:=`(irrigation_below, ifelse(irrigation_sum < subsidy_threshold, subsidy_threshold - irrigation_sum,
                                            0))]
  ...
  for (i in 1:max(KS_DSSAT_2$group)) {
    ...
    soil_type[, `:=`(Soil_Type, KS_DSSAT_2[group ==
                                             i, unique(SOIL_ID)])]
    ...
    data.table::setnames(well_capacity_data, old = default_well_capacity_col_name,
                         "Well_capacity")
    WSTAT = data.table(Well_ID = well_capacity_data$Well_ID, weather_station = KS_DSSAT_2[group ==
                                                                                            i, unique(WSTA)])
    ...
    setkey(foo_irr, Well_ID, tot_acres, SDAT, quarter, CR,
           PAW)
    # foo_irr[, `:=`(group_1, .GRP), by = c("Well_ID",
    #                                       "tot_acres", "SDAT")]
    ...
    # foo_dt_all = foo_dt_all[, .(Well_ID, tot_acres, quarter,
    ...
    lookup_table_quarter = lookup_table_quarter[, .(Well_capacity,
                                                    SOIL_ID, WSTA, tot_acres, quarter, SDAT, CR, PAW,
                                                    irrigation_quarter = irrigation, yield_kg_ac, profit_quarter = profit)]
