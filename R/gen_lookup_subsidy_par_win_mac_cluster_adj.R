...
)
{
  ...
  for (i in 1:max(KS_DSSAT_2$group)) {
    ...
    if (Sys.info()[1] == "Windows") {
      ...
      foo_dt_all_1 <- parLapply(cl, 1:floor(aa/4),                     FN_optim3)
      foo_dt_all_2 <- parLapply(cl, (floor(aa/4)+1):(2*floor(aa/4)),   FN_optim3)
      foo_dt_all_3 <- parLapply(cl, (2*floor(aa/4)+1):(3*floor(aa/4)), FN_optim3)
      foo_dt_all_4 <- parLapply(cl, (3*floor(aa/4)+1):aa,              FN_optim3)
      stopCluster(cl)
    } else {
      print(Sys.info()[1])
